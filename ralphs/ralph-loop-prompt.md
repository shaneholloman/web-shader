...
```
[number]-feature-name/
├── index.ts           # Main script with LoopAgent configuration
├── package.json       # Package definition
├── tsconfig.json      # TypeScript config
├── .env              # API keys (copy from another ralph folder)
├── .progress.md      # Generated by agent during execution
└── .brain/           # Generated by agent if using brainRule
    ├── index.md
    └── conventions.md
```
